...
function LoginScreen({ setIsLoggedIn, setUserDetails, setMessage }) {
  ...
  return (
    <div className="login-container">
      <h2 className="login-title">Assignment 15 - Profile Update with Avatar Upload</h2>
      ...
      <div className="instructions">
        ...
        <p>• Login to auth.dnjs.lk and update your profile/avatar</p>
...
function ProfileScreen({ userDetails, setIsLoggedIn, setUserDetails, setMessage }) {
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      // You may need to append other fields based on Network tab findings
      // formData.append('profile_picture', selectedFile);

      // Upload avatar - replace with actual endpoint found in Network tab
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      // Update profile - replace with actual endpoint found in Network tab
  ...
  return (
    <div className="login-container">
      ...
      <div className="user-details">
        ...
        <div className="form-group">
          <label className="form-label">Select Avatar Image:</label>
          <input
            id="avatar-input"
            type="file"
            accept="image/*"
        ...
        <button 
          ...
          className={avatarLoading || !selectedFile ? 'login-button disabled' : 'login-button'}
      ...
      <div className="instructions">
        ...
        <p>3. Login and update your avatar while Network tab is open</p>
        <p>4. Find the avatar upload API request in Network tab</p>
        <p>5. Check the URL, method (POST/PUT), and request payload (FormData)</p>
        <p>6. Update the API call in handleAvatarUpload() with correct endpoint</p>
        <p>7. Also update profile API endpoint in handleUpdateProfile() if needed</p>
        
        <h3>Avatar Upload Notes:</h3>
        <p>• File input only accepts image files (image/*)</p>
        <p>• Uses FormData for multipart/form-data upload</p>
        <p>• Authorization header included in upload request</p>
...
function Assignment_15() {
  ...
  return (
    <div>
      {message && (
        <div className={message.includes('successful') || message.includes('Logged in') || message.includes('updated') || message.includes('uploaded') || message.includes('Selected') ? 'success-message' : 'error-message'}>
          <strong>{message.includes('successful') || message.includes('Logged in') || message.includes('updated') || message.includes('uploaded') || message.includes('Selected') ? 'Success:' : 'Error:'}</strong> {message}
        </div>
      )}
